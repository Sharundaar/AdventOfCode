...
void output_area_to_image( const vector<entry>& entries, uint wx_min, uint wy_min, const vector<uint>& area, const vector<uint>& area_dist_sum, uint w, const char* img )
{
    int x, y;
    /* size of the image */
    ...
    for (y = 0; y < y_max; ++y) {
        for (x = 0; x < x_max; ++x) {
            ...
            if( point_is_entry( x+wx_min, y+wy_min, entries ) )
                pixel = TO_PX32( 255, 255 );
            else if( area[y*w+x] == CONFLICTED_AREA )
                pixel = TO_PX32( 0, 255 );
            else if( area_dist_sum[y*w+x] < DIST_THREASHOLD )
            {
                auto color = (uint)color_palette[area[y*w+x]]; // 8 bit color
                pixel = TO_PX32( 0b01101001 ^ color, 127 );
            }
            else
            {
                auto color = (uint)color_palette[area[y*w+x]]; // 8 bit color
...
int main()
{
    ...
    if( file )
    {
        ...
        vector<uint> area_dist_sum( w*h );

        uint searched_region_size = 0;
        ...
        for( uint i=0; i<area.size(); ++i )
        {
            ...
            if( area_dist_sum[i] < DIST_THREASHOLD )
                searched_region_size++;
        }

        output_area_to_image( entries, min_x, min_y, area, area_dist_sum, w, "img.bmp");

        cout << searched_region_size << endl;
    }
