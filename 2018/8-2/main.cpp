...
using NodePool = pool<node, 0x10000>;
...
int compute_node_value( const node* node )
{
    ...
    for( int i=0; i<node->metadata.size(); ++i )
    {
        int node_ref = node->metadata[i]-1;
        if( node_ref < node->childs.size() )
            computed_value += compute_node_value( node->childs[node_ref] );
    }
...
int main()
{
    ...
    if( *it != '\0' )
    {
        cout << "Didn't reach end of file." << endl;
        return 0;
    }

    // print_node( root );
    cout << compute_node_value( root ) << endl;
}