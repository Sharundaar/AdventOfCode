...
static u64 get_mask( Mask mask[36], Mask m )
{
    u64 one_mask = 0;
    for( int i=0; i<36; ++i )
    {
        if( mask[i] == m ) one_mask += 1;
        if( i < 35 )
            one_mask = one_mask << 1;
    }
...
int main( int argc, char* argv[] )
{
    ...
    if( file.is_open() )
    {
        ...
        for( int i=0; i<entries.size(); ++i )
        {
            ...
            u64 one_mask = get_mask( entry.mask, SET_ONE );
            u64 x_mask = get_mask( entry.mask, IGNORE );
            for( const Operation& op : entry.ops )
            {
                u64 position = op.position;
                int first_x_bit_position = -1;
                u64 search_pattern = 1;
                while(  )
                assert( op.position < ARRAY_SIZE( memory ) );
                memory[op.position] = ( op.value | one_mask );
            }
        }
